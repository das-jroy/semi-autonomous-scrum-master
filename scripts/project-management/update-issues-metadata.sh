...
get_custom_field_ids() {
    ...
    # Extract field IDs - updated for generic field names
    PRIORITY_FIELD_ID=$(echo "$fields_response" | jq -r '.data.node.fields.nodes[] | select(.name == "Priority Level") | .id')
    CATEGORY_FIELD_ID=$(echo "$fields_response" | jq -r '.data.node.fields.nodes[] | select(.name == "Component Category") | .id')
    COMPLEXITY_FIELD_ID=$(echo "$fields_response" | jq -r '.data.node.fields.nodes[] | select(.name == "Complexity") | .id')
    REVIEW_FIELD_ID=$(echo "$fields_response" | jq -r '.data.node.fields.nodes[] | select(.name == "Review Status") | .id')
    
    # Get option IDs for single select fields - updated for generic options
    PRIORITY_HIGH_ID=$(echo "$fields_response" | jq -r '.data.node.fields.nodes[] | select(.name == "Priority Level") | .options[] | select(.name == "P1 - High Priority") | .id')
    PRIORITY_MEDIUM_ID=$(echo "$fields_response" | jq -r '.data.node.fields.nodes[] | select(.name == "Priority Level") | .options[] | select(.name == "P2 - Medium Priority") | .id')
    PRIORITY_LOW_ID=$(echo "$fields_response" | jq -r '.data.node.fields.nodes[] | select(.name == "Priority Level") | .options[] | select(.name == "P3 - Low Priority") | .id')
    ...
    REVIEW_REQUIRED_ID=$(echo "$fields_response" | jq -r '.data.node.fields.nodes[] | select(.name == "Review Status") | .options[] | select(.name == "Required") | .id')
    REVIEW_NOT_REQUIRED_ID=$(echo "$fields_response" | jq -r '.data.node.fields.nodes[] | select(.name == "Review Status") | .options[] | select(.name == "Not Required") | .id')
    
    # Get Component Category option IDs
    CATEGORY_FRONTEND_ID=$(echo "$fields_response" | jq -r '.data.node.fields.nodes[] | select(.name == "Component Category") | .options[] | select(.name == "Frontend") | .id')
    CATEGORY_BACKEND_ID=$(echo "$fields_response" | jq -r '.data.node.fields.nodes[] | select(.name == "Component Category") | .options[] | select(.name == "Backend") | .id')
    CATEGORY_DATABASE_ID=$(echo "$fields_response" | jq -r '.data.node.fields.nodes[] | select(.name == "Component Category") | .options[] | select(.name == "Database") | .id')
    CATEGORY_API_ID=$(echo "$fields_response" | jq -r '.data.node.fields.nodes[] | select(.name == "Component Category") | .options[] | select(.name == "API") | .id')
    CATEGORY_TESTING_ID=$(echo "$fields_response" | jq -r '.data.node.fields.nodes[] | select(.name == "Component Category") | .options[] | select(.name == "Testing") | .id')
    CATEGORY_INFRASTRUCTURE_ID=$(echo "$fields_response" | jq -r '.data.node.fields.nodes[] | select(.name == "Component Category") | .options[] | select(.name == "Infrastructure") | .id')
    CATEGORY_SECURITY_ID=$(echo "$fields_response" | jq -r '.data.node.fields.nodes[] | select(.name == "Component Category") | .options[] | select(.name == "Security") | .id')
    CATEGORY_DOCUMENTATION_ID=$(echo "$fields_response" | jq -r '.data.node.fields.nodes[] | select(.name == "Component Category") | .options[] | select(.name == "Documentation") | .id')
    ...
    echo "Review Field ID: $REVIEW_FIELD_ID"
    ...
    echo "Review Required ID: $REVIEW_REQUIRED_ID"
    echo "Review Not Required ID: $REVIEW_NOT_REQUIRED_ID"
    echo ""
}
...
get_category_option_id() {
    ...
    case "$category" in
        "Frontend")
            echo "$CATEGORY_FRONTEND_ID"
            ;;
        "Backend")
            echo "$CATEGORY_BACKEND_ID"
            ;;
        "Database")
            echo "$CATEGORY_DATABASE_ID"
            ;;
        "API")
            echo "$CATEGORY_API_ID"
            ;;
        "Testing")
            echo "$CATEGORY_TESTING_ID"
            ;;
        "Infrastructure")
            echo "$CATEGORY_INFRASTRUCTURE_ID"
            ;;
        "Security")
            echo "$CATEGORY_SECURITY_ID"
            ;;
        "Documentation")
            echo "$CATEGORY_DOCUMENTATION_ID"
            ;;
        *)
            echo "$CATEGORY_INFRASTRUCTURE_ID"  # Default to Infrastructure
            ;;
    esac
}
