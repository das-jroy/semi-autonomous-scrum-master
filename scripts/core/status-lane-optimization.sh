...
echo "🎯 Status Lane Optimization for Azure InfraWeave Project"
echo "======================================================="
...
for item in $(gh project item-list 3 --owner dasdigitalplatform --format json | jq -r '.items[] | select(.status == "Todo") | .id'); do
