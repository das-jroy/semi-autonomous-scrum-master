...
validate_roadmap_integration() {
    ...
    if command -v gh &> /dev/null && gh auth status &> /dev/null; then
        local issue_count=$(gh issue list --repo "$REPO_OWNER/vanguard-az-infraweave-catalog" --limit 100 --json number | jq length 2>/dev/null || echo "0")
...
- **Project Board:** https://github.com/orgs/dasdigitalplatform/projects/3
...
main() {
    ...
    echo ""
    echo -e "${YELLOW}Your Azure InfraWeave project now has:${NC}"
